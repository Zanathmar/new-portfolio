...
const Experience = () => {
  ...
  const experiences = [
    ...
    { company: "Kementerian Komunikasi dan Digital (Komdigi)", role: "Sistem Informasi Intern", period: "August - October 2026", type: "Internship", description: "Developing internal information systems as part of the Sistem Informasi division.", status: "Ongoing", duration: "3 months", technologies: [] },
  ];
            ...
            {experiences.map((exp, index) => {
              ...
              return (
                ...
                >
                  ...
                  <div className={`md:ml-20 bg-white border rounded-xl sm:rounded-2xl overflow-hidden hover:shadow-xl transition-all duration-300 group ${isInternship ? 'border-black border-2' : 'border-gray-200'}`}>
                    <div className={`h-1 bg-black ${hoveredIndex === index ? 'opacity-100' : 'opacity-0'} transition-opacity duration-300`}></div>
                    <div className="p-4 sm:p-6 lg:p-8">
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2">
                            <span className={`inline-block px-2.5 py-0.5 sm:px-3 sm:py-1 text-[10px] sm:text-xs font-medium rounded-full uppercase tracking-wide ${isInternship ? 'bg-white text-black border-2 border-black' : 'bg-black text-white'}`}>{exp.type}</span>
                            {isInternship && exp.status && (
                              <span className="inline-flex items-center gap-1 px-2.5 py-0.5 sm:px-3 sm:py-1 bg-green-100 text-green-800 text-[10px] sm:text-xs font-medium rounded-full uppercase tracking-wide">
